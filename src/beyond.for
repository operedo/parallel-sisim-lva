...
c-----------------------------------------------------------------------
      ...
      real, parameter :: UNEST=-1.0
      !parameter(EPSLON=1.0e-20,UNEST=-1.0)
      !dimension ccut(nccut),ccdf(nccut),cut(1),cdf(1)
      !real, intent(in)::      utpar,mpar,ltpar
      !integer,intent(in)::   ltail,utail,middle

      integer, intent(in)::ivtype,nccut,ncut
      real, intent(in) :: ccut(nccut),ccdf(nccut),cut(1),cdf(1)
      real*8, intent(in) :: zmin,zmax
      integer,intent(in)::ltail,middle,utail
      real,intent(in)::utpar,ltpar,mpar 
      real*8,intent(inout)::zval
      real,intent(inout)::cdfval
      integer,intent(inout)::ierr

      integer cclow,cchigh
      integer ipart,i,idat,iupp,ilow
      real cum
      real*8 powr,temp,lambda
      external powint
...
c
      ierr  = 1
      !write(*,*)'HOLA',ccdf(:)
      !write(*,*)'HOLA',zval,UNEST,cdfval
      !write(*,*)zval.gt.UNEST
      !write(*,*)cdfval.gt.UNEST
      !write(*,*)zval.le.UNEST
      !write(*,*)cdfval.le.UNEST
...
c
      if(ivtype.eq.0) then
            ...
            do i=1,nccut
                  cum = cum + ccdf(i)
                  !write(*,*)'i',i,'cum',cum,'ccdf(i)',ccdf(i)
                  if(cdfval.le.cum) then
                         zval = ccut(i)
                         !write(*,*)'zval',zval,'ccut(',i,')',ccut(i)
                         return
                  endif
            end do
            return
